...
Reads configuration from environment variables (no config file required):
    AUNIX_TARGET_ID    - integer target id
    AUNIX_AGENT_TOKEN  - opaque bearer token from the dashboard
    AUNIX_API_URL      - e.g. https://aunix-api.onrender.com/api

Designed to be run via curl-pipe-python. The dashboard issues a one-line
command containing all three variables; the user copies and runs it once
on the target.
"""
...
def show_rotated_token(new_token: str) -> None:
    """Display the rotated token so the user can re-fetch a fresh command.

    Because the agent is invoked via curl-pipe-python with no on-disk
    config, the rotated token cannot be persisted automatically. The user
    fetches a fresh command from the dashboard for the next scan.
    """
    print()
    print("=" * 60)
    print("Agent token rotated. The previous token is now invalid.")
    print("Generate a fresh command from the dashboard before the next scan.")
    print("=" * 60)
...
def load_config() -> dict:
    """Read target_id, agent_token, and api_url from environment variables."""
    target_id = os.getenv("AUNIX_TARGET_ID")
    agent_token = os.getenv("AUNIX_AGENT_TOKEN")
    api_url = os.getenv("AUNIX_API_URL")

    missing = []
    if not target_id:
        missing.append("AUNIX_TARGET_ID")
    if not agent_token:
        missing.append("AUNIX_AGENT_TOKEN")
    if not api_url:
        missing.append("AUNIX_API_URL")

    if missing:
        sys.exit(
            "Missing required environment variable(s): "
            + ", ".join(missing)
            + "\nRun the agent using the command shown in the AUNIX dashboard."
        )

    try:
        target_id_int = int(target_id)
    except ValueError:
        sys.exit(f"AUNIX_TARGET_ID must be an integer, got: {target_id!r}")
